...
function hasGeminiKey() {
    return Boolean(AI_CONFIG.apiKey && AI_CONFIG.apiKey.trim() !== '');
}
...
async function validarComIA({ nome, genero, sexualidade, descricao }) {
    const prompt = `Use apenas a autodeclaração explícita e o texto livre da pessoa. Ignore a opção de sexualidade escolhida e qualquer estereótipo (nome, gênero, roupas, hobbies, memes, beijo, parada, esportes, aparência ou cultura). Se a pessoa não declarou claramente algo no texto, responda indefinido. Responda apenas JSON válido no formato {"resultado":"gay|lesbica|nao-gay|indefinido"}. Texto voluntário: ${descricao}.`;

    if (!AI_CONFIG.enabled || !hasGeminiKey()) {
        return { resultado: 'indefinido' };
    }

    try {
        const url = `${AI_CONFIG.endpoint}?key=${encodeURIComponent(AI_CONFIG.apiKey)}`;
        ...
        if (!response.ok) throw new Error('Erro da API de IA');

        const data = await response.json();
        const mensagem = data.candidates?.[0]?.content?.parts
            ?.map((part) => part.text)
            .join('') || '{}';

        const textoLimpo = mensagem.replace(/```json|```/gi, '').trim();
        const jsonMatch = textoLimpo.match(/\{.*\}/s);
        const textoJson = jsonMatch ? jsonMatch[0] : textoLimpo;
        const parsed = JSON.parse(textoJson);

        if (parsed.resultado) return { resultado: parsed.resultado };
    } catch (error) {
        console.warn('IA indisponível, usando regra local:', error);
    }

    return { resultado: 'indefinido' };
}

function calcularResultadoLocal(descricao = '') {
    const autodeclaracao = descricao.toLowerCase();

    if (/(ainda estou descobrindo|em descoberta|não sei|nao sei|não tenho certeza|nao tenho certeza|prefiro não responder|prefiro nao responder)/i.test(autodeclaracao)) {
        return 'indefinido';
    }

    if (/(sou|me identifico como|me considero|me reconheço como)\s+(um homem\s+)?gay\b|sou homossexual\b/i.test(autodeclaracao)) {
        return 'gay';
    }

    if (/(sou|me identifico como|me considero|me reconheço como)\s+(uma mulher\s+)?lésbica\b|sou lesbica\b/i.test(autodeclaracao)) {
        return 'lesbica';
    }

    if (/(sou|me identifico como|me considero|me reconheço como)\s+heterossexual\b|sou heterossexual\b/i.test(autodeclaracao)) {
        return 'nao-gay';
    }
...
function mostrarOrientacaoSelecionada() {
    ...
    const nomes = {
        gay: 'Gay',
        lesbica: 'Lésbica',
        'nao-gay': 'Heterossexual',
        indefinido: 'Ainda em descoberta'
    };
    const confirmacao = document.createElement('p');
    confirmacao.className = 'result-confirmation';
    confirmacao.textContent = `Autodeclaração registrada: ${nomes[orientacao] || 'Não classificada'}`;
...
if (form) {
    form.addEventListener('submit', async (event) => {
        ...
        if (!nome || !idade || !genero || (!descricao.trim() && !contextoExtra.trim())) {
            textoResultado.textContent = 'Preencha seu nome, idade, gênero e ao menos uma descrição antes de enviar.';
            resultado.classList.add('active');
            return;
        }

        let tipoResultado = 'indefinido';

        if (sexualidade === 'descobrindo' || sexualidade === 'nao-responder') {
            tipoResultado = 'indefinido';
        } else {
            tipoResultado = calcularResultadoLocal(descricao || contextoExtra);
        }

        if (tipoResultado === 'indefinido') {
            const resultadoIA = await validarComIA({ nome, genero, sexualidade, descricao: descricao || contextoExtra });
            tipoResultado = resultadoIA.resultado || tipoResultado;
        }
        ...
        if (tipoResultado === 'gay') {
            window.location.replace('gay.html');
            return;
        }

        if (tipoResultado === 'lesbica') {
            window.location.replace('lesbica.html');
            return;
        }
        ...
        window.location.replace('nao-gay.html');
    });
}
