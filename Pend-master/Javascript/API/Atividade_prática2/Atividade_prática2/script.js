...
async function validarComIA({ nome, genero, sexualidade, descricao }) {
    const prompt = `Normalize somente a autodeclaração explícita da pessoa. Não use nome, gênero, idade, roupas, hobbies, memes, beijo, frequência em paradas ou qualquer estereótipo para inferir orientação sexual. Se a pessoa não declarou claramente uma orientação, responda indefinido. Responda apenas JSON válido no formato {"resultado":"gay|lesbica|nao-gay|indefinido"}. Sexualidade declarada: ${sexualidade}; Texto voluntário: ${descricao}.`;
...
function calcularResultadoLocal(sexualidade, privacidade, descricao = '') {
    ...
    if (/(sou|me identifico como|me considero|me reconheço como)\s+heterossexual\b/i.test(autodeclaracao)) {
        return 'nao-gay';
    }

    if (sexualidade === 'gay') return 'gay';
    if (sexualidade === 'lesbica') return 'lesbica';
    if (sexualidade === 'heterossexual') return 'nao-gay';
    if (privacidade === 'nao-responder') return 'indefinido';
    return 'indefinido';
}
...
if (form) {
    form.addEventListener('submit', async (event) => {
        ...
        const respostasExtras = Array.from(form.querySelectorAll('.field select, .field textarea'))
            .map((controle) => controle.value.trim())
            .filter(Boolean);
        const contextoExtra = respostasExtras.join(' ');
        const descricao = document.getElementById('descricao').value.trim();

        if (!nome || !idade || !genero || !sexualidade || !contextoExtra.trim()) {
            textoResultado.textContent = 'Preencha todos os campos antes de enviar.';
            resultado.classList.add('active');
            return;
        }

        let tipoResultado = calcularResultadoLocal(sexualidade, '', descricao);

        if (tipoResultado === 'indefinido') {
            const resultadoIA = await validarComIA({ nome, genero, sexualidade, descricao: contextoExtra });
